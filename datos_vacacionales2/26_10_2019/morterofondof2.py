...
datos2Ffondo=ascii.read('26-10-2019-PX5-HPGe-300s-fuentes2-fondo.MCA', data_start=0)

g=len(datos2Ffondo)

cuentas2Ffondo=np.zeros(g)
#canalM11=np.zeros(g)
ejeX2Ffondo=np.zeros(g)
for i in range(0,g):
    cuentas2Ffondo[i]=datos2Ffondo[i][0]
    #canal[i]=datos[i][0]
    ejeX2Ffondo[i]=i
...
M1=int(max(cuentas2Ffondo[1000:1120]))


for i in range(1000,1120):
	if cuentas2Ffondo[i]==M1:
		miu1=ejeX2Ffondo[i]

sigma1=math.sqrt(miu1)


cuentas1=cuentas2Ffondo[1000:1120]
eje1=ejeX2Ffondo[1000:1120]
...
M2=int(max(cuentas2Ffondo[1320:1420]))


for i in range(1320,1420):
	if cuentas2Ffondo[i]==M2:
		miu2=ejeX2Ffondo[i]

sigma2=math.sqrt(miu2)


cuentas2=cuentas2Ffondo[1320:1420]
eje2=ejeX2Ffondo[1320:1420]
...
#################### 22Na - 1274keV #####################
#########################################################
M3=int(max(cuentas2Ffondo[2620:2680]))


for i in range(2620,2680):
	if cuentas2Ffondo[i]==M3:
		miu3=ejeX2Ffondo[i]

sigma3=math.sqrt(miu3)


cuentas3=cuentas2Ffondo[2620:2680]
eje3=ejeX2Ffondo[2620:2680]
...
LIVE_MODE - 0
PRESET_TIME - 0
LIVE_TIME - 288.150000
REAL_TIME - 300.000000
START_TIME - 10/26/2019 09:58:28
SERIAL_NUMBER - 0
<<DATA>>
---------------------------------------------------------
<<END>>
